...
public class GestioneConcessionario <T> {
	...
	public GestioneConcessionario() throws Exception {
	...
	private void comandoCancellaVeicolo(Scanner scan) {
		...
		if(GestioneConcessionario.ripetereComando(scan,"Sicuro di voler eliminare il veicolo "+ numeroVeicolo +"? Scrivi 1 per \"SI\" E 2 per \" no\" " )) {
	...
	private void comandoModificaVeicolo(Scanner scan) {
		...
		try {
			...
			if(GestioneConcessionario.ripetereComando(scan,"Vuoi ripetere l'operazione? 1: Ripetere 0: Uscire")) {
				comandoModificaVeicolo(scan);
			}
	...
	private void comandoAggiungiVeicolo(Scanner scan) {
		...
		try {
			scelta=GestioneConcessionario.ripetereComando(scan, "Inserisci 0 per aggiungere un Automobile o inserisci 1 per aggiungere una Macchina Agricola");
			scan.nextLine();
	...
	private static boolean ripetereComando(Scanner scan,String msg) {
		...
		try {
			...
			//questa dovrebbe essere un'eccezione ma la gestiamo con un if
				...
				scan.nextLine();
				GestioneConcessionario.ripetereComando(scan,msg);
			}
		} catch (InputMismatchException e) {
			System.out.println("Hai sbagliato input, RIPROVA!!");
			scan.nextLine();
